...
<!--  <div class="msg_empty">[{ oxmultilang ident="BASKET_EMPTYSHIPPINGCART" }]<br />
<br />
</div>-->
    ...
    <form action="[{ $oViewConf->getSslSelfLink() }]" method="post">
      <div>
          ...
          <div class="right arrowright">
              <input id="test_BasketNextStepTop" class="btn" type="submit" value="[{ oxmultilang ident="BASKET_NEXTSTEP" }]">
          </div>
      </div>
    </form>
    [{/if}]
 ...
 <form name="basket[{ $basketindex }]" action="[{ $oViewConf->getSelfActionLink() }]" method="post">
  ...
  <table class="basket">
    ...
    <div class="basket-header">
          <p class="basket-head-article">[{ oxmultilang ident="BASKET_ARTICEL" }]</p>
          <p class="basket-head-quantity">[{ oxmultilang ident="BASKET_QUANTITY" }]</p>
          <p class="basket-head-unitprice">[{ oxmultilang ident="BASKET_UNITPRICE" }]</p>
          <p class="basket-head-tax">[{ oxmultilang ident="BASKET_TAX" }]&nbsp;&nbsp;</p>
          <p class="basket-head-total">[{ oxmultilang ident="BASKET_TOTAL" }]</p>
    </div><!--basket-header-->
    ...
    <tr valign="top">
      <!-- product image -->
      <td>
          <input id="test_removeCheck_[{$basketitem->getProductId()}]_[{$smarty.foreach.test_Contents.iteration}]" type="checkbox" class="basket-checkbox" name="aproducts[[{ $basketindex }]][remove]" value="1">
      </td>
      <td class="basketnewM" >
      ...
      <td align="left" class="quantity-manager">
      ...
      <td align="right" id="test_basket_TotalPrice_[{$basketitem->getProductId()}]_[{$smarty.foreach.test_Contents.iteration}]" class="totalprice">
         [{if !$basketitem->isBundle() || !$basketitem->isDiscountArticle()}]
    ...
    <!--  basket update/delete buttons  -->
     <tr class="sumrow">
       <td class="brd" valign="top">
          <input type="checkbox" class="basket-checkbox-erease" name="checkAll" onClick="oxid.checkAll(this, 'aproducts')" title="[{ oxmultilang ident="BASKET_SELECT_ALL" }]">
       </td>
       <td colspan="6">

        <div class="remove-from-basket">
		...
		<div class="update-basket">
             <input id="test_basketUpdate" class="btn" type="submit" name="updateBtn" value="[{ oxmultilang ident="BASKET_UPDATE" }]">
		</div>
     </tr>
    ...
    [{if !$oxcmp_basket->getDiscounts() }]
     ...
     <tr class="sumrow green">
       <td class="brd"></td>
       <td colspan="5" class="sumdesc">[{ oxmultilang ident="BASKET_TAX1" }]&nbsp;[{ $key }]%</td>
    ...
    [{if $oxcmp_basket->getDiscounts() }]
      ...
      [{foreach from=$oxcmp_basket->getProductVats() item=VATitem key=key }]
       <tr class="sumrow green">
         <td class="brd"></td>
         <td colspan="5" class="sumdesc">[{ oxmultilang ident="BASKET_TAX1" }] [{ $key }][{ oxmultilang ident="BASKET_TAX1" }]</td>
         <td id="test_basketVAT_[{$key}]" align="right">[{ $VATitem }]&nbsp;[{ $currency->sign }]</td>
         <td></td>
       </tr>
      [{/foreach }]
    ...
    <tr class="sumrow total total-price">
      <td class="brd"></td>
         ...
         [{if $oEr->getErrorClassType() == 'oxVoucherException'}]
          ...
          <form name="voucher" action="[{ $oViewConf->getSelfActionLink() }]" method="post" class="left">
			  ...
			  <strong id="test_VoucherHeader" class="boxhead">[{ oxmultilang ident="BASKET_REDEEMCOUPON" }]</strong>
                  ...
                  <input type="text" size="20" name="voucherNr" class="voucher-number">
                  <div class="send-voucher">
				  <input class="btn" id="test_basketVoucherAdd" class="btn" type="submit" value="[{ oxmultilang ident="BASKET_SUBMITCOUPON" }]">
				  </div>
                  <input type="hidden" name="CustomError" value='basket'>
              </div>
          </form>
  ...
  <div class="bar prevnext bottom">
    ...
    <form action="[{ $oViewConf->getSslSelfLink() }]" method="post">
      <div>
          ...
          <input type="hidden" name="cl" value="user">
          <div class="right arrowright">
              <input id="test_BasketNextStepBottom" class="btn" type="submit" value="[{ oxmultilang ident="BASKET_NEXTSTEP" }]">
          </div>
      </div>
    </form>
    [{/if}]
