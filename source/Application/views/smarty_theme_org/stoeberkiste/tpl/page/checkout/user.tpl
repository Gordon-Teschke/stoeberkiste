...
[{include file="_header.tpl" title=$template_title location=$template_title seopage="checkout"}]
...
[{assign var="_blshownoregopt" value=$oView->getShowNoRegOption()}]
  ...
  [{ if !$oxcmp_user && !$oView->getLoginOption() && !$oView->isConnectedWithFb() }]
    [{if $_blshownoregopt }]
      <div class="left user-option-one">
          <strong class="useroptboxhead">[{ oxmultilang ident="USER_OPTION1" }]</strong>
          <div class="useroptbox">
              <b>[{ oxmultilang ident="USER_ORDERWITHOUTREGISTER1" }]</b><br />
              <small>[{ oxmultilang ident="USER_ORDERWITHOUTREGISTER2" }]</small><br><br>
              ...
              <form action="[{ $oViewConf->getSslSelfLink() }]" method="post">
                <div>
                    ...
                    <input id="test_UsrOpt1" type="submit" name="send" value="[{ oxmultilang ident="USER_NEXT" }]" class="btn">
                </div>
              </form>
          </div>
      </div>
    [{/if}]
      <div class="left user-option-two">
          ...
          <div class="useroptbox[{if !$_blshownoregopt }]big[{/if}]">
              <b>[{ oxmultilang ident="USER_ALREADYCUSTOMER" }]</b><br>
              <small>([{ oxmultilang ident="USER_PLEASELOGIN" }])</small><br><br>
              ...
              <form action="[{ $oViewConf->getSslSelfLink() }]" method="post">
                <div>
                    ...
                    <input class="user-reg-input" id="test_UsrOpt2_usr" type="text" name="lgn_usr" value="" size="25"><br>
                    <span class="fs11"><b>[{ oxmultilang ident="USER_PWD" }]</b></span><br>
                    <input class="user-reg-input" id="test_UsrOpt2_pwd" type="password" name="lgn_pwd" value="" size="25"><br>
                    <div class="user-reg">
					<input id="test_UsrOpt2" type="submit" name="send" value="[{ oxmultilang ident="USER_LOGIN" }]" class="btn"><br>
					</div>
                    <a class="btn" id="test_UsrOpt2_forgotPwd" rel="nofollow" href="[{ oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=forgotpwd" }]" class="link">[{ oxmultilang ident="USER_FORGOTPWD" }]</a><br>
                 </div>
              </form>
          </div>
      </div>

      <div class="left user-option-three">
          ...
          <div class="useroptbox[{if !$_blshownoregopt }]big[{/if}]">
              <b>[{ oxmultilang ident="USER_OPENPERSONALACCOUNT1" }]</b><br>
              ...
              <form action="[{ $oViewConf->getSslSelfLink() }]" method="post">
                <div>
                    ...
                    <div class="user-option-three-submit">
					<input id="test_UsrOpt3" type="submit" name="send" value="[{ oxmultilang ident="USER_LOGIN2" }]" class="btn">
					</div>
                </div>
              </form>
          </div>
  ...
  [{else}]
      ...
      <div class="box info">
          ...
          <form action="[{ $oViewConf->getSslSelfLink() }]" method="post">
            <div>
                ...
                <span class="btn1"><input id="test_UsrOpt2UpdateAccount" type="submit" name="send" value="[{ oxmultilang ident="USER_UPDATE_ACCOUNT" }]" class="btn"></span><br><br>
             </div>
          </form>
      </div>
    [{/if}]
    ...
    <form action="[{ $oViewConf->getSslSelfLink() }]" name="order" method="post">
      ...
      [{else}]
        <div class="bar prevnext order">
            <div class="right arrowright">
                <input class="btn" id="test_UserNextStepTop" name="userform" type="submit" value="[{ oxmultilang ident="USER_NEXTSTEP" }]">
            </div>
        </div>
      [{/if}]
        ...
        <div class="box info">
            ...
            <table class="form" width="100%">
              ...
              <tr>
                ...
                <td class="form-input"><input id="userLoginName" type="text" name="lgn_usr" value="[{$oView->getActiveUsername()}]" size="37">&nbsp;<span class="req">*</span></td>
              </tr>
              <tr>
      ...
      <strong class="boxhead">[{ oxmultilang ident="USER_SEND" }]</strong><br /><br />
      <div class="box info">
          <b>[{ oxmultilang ident="USER_BILLINDADDRESS" }]</b><br /><small>[{ oxmultilang ident="USER_COMPLETEALLMARKEDFIELDS" }]</small><br /><br />
          <div class="dot_sep"></div>
...
*}]
            <tr class="td_sep">
              ...
              <td>
                ...
                [{/if}]
                <textarea class="form-text" cols="60" rows="7" name="order_remark">[{$order_remark}]</textarea>
              </td>
            </tr>
		...
		<div class="change-shipping">
          [{if !$oView->showShipAddress()}]
            <span class="btn1"><input type="submit" name="blshowshipaddress" value="[{ oxmultilang ident="USER_DIFFERENTSHIPPINGADDRESS" }]" class="btn"></span><br /><br />
          [{else}]
            <span class="btn1"><input type="submit" name="blhideshipaddress" value="[{ oxmultilang ident="USER_DISABLESHIPPINGADDRESS" }]" class="btn"></span><br /><br />
          [{/if}]
		</div>
          <div class="fs10 def_color_1"><span class="req">[{ oxmultilang ident="USER_NOTE" }]</span> [{ oxmultilang ident="USER_DIFFERENTDELIVERYADDRESS" }]</div><br /><br />
          [{if $oView->showShipAddress()}]
            [{if $oxcmp_user}]
            ...
            <table class="form" width="90%">
              ...
              <tr>
                ...
                <td>
                  <select class="form-country" name="oxaddressid" onchange="oxid.form.set('reloadAddress', this.value === '-1' ? 1 : 2);oxid.form.reload(this.value === '-1','order','user','');oxid.form.clear(this.value !== '-1','order',/oxaddress__/);">
                  ...
                  <noscript>
                    <span class="btn1"><input id="test_accUserReloadAddress" class="btn" type="submit" name="reloadaddress" value="[{ oxmultilang ident="USER_ADDRESSES_SELECT" }]"></span>
                  </noscript>
                </td>
              </tr>
              <tr>
              ...
              <tr>
                <td><label>[{ oxmultilang ident="USER_COUNTRY2" }]</label></td>
                 <td>
                  <select class="form-country id="delCountrySelect" name="deladr[oxaddress__oxcountryid]">
                      <option value="">-</option>
                      [{assign var="blCountrySelected" value=false}]
                      [{assign var="sCountrySelect" value=""}]
                      [{foreach from=$oViewConf->getCountryList() item=country key=country_id}]
      ...
      [{else}]
        <div class="bar prevnext">
            <div class="right arrowright">
                <input class="btn" id="test_UserNextStepBottom" name="userform" type="submit" value="[{ oxmultilang ident="USER_NEXTSTEP" }]">
            </div>
        </div>
      [{/if}]
