    ...
    <div class="box info">
        <form action="[{ $oViewConf->getSslSelfLink() }]" name="shipping" id="shipping" method="post">
          <div>
              ...
              <div class="left">
                <select class="form-country" name="sShipSet" onChange="JavaScript:document.forms.shipping.submit();">
                ...
                <noscript>
                  <div>
                    <span class="btn1"><input class="btn" type="submit" value="[{ oxmultilang ident="PAYMENT_UPDATESHIPPING" }]" ></span>
                  </div>
                </noscript>
              </div>
        ...
        <form action="[{ $oViewConf->getSslSelfLink() }]" name="order" method="post">
          <div>
            ...
            [{if $oView->getPaymentList()}]
              ...
              <div class="box info">
				<strong id="test_PaymentHeader" class="" style="clear:both;float:left;">[{ oxmultilang ident="PAYMENT_PAYMENT" }]</strong>
              <table class="form" style="clear:both;float:left;padding-right:16px;">
                ...
                [{foreach key=sPaymentID from=$oView->getPaymentList() item=paymentmethod name=PaymentSelect}]
                  ...
                  [{if $sPaymentID == "oxidcashondel"}]
                    <tr onclick="oxid.form.select('paymentid',[{$inptcounter}]);">
                      <td><input style="float:left;width:auto;padding-right:16px;" id="test_Payment_[{$sPaymentID}]" type="radio" name="paymentid" value="[{$sPaymentID}]" [{if $oView->getCheckedPaymentId() == $paymentmethod->oxpayments__oxid->value}]checked[{/if}]></td>
                  ...
                  [{elseif $sPaymentID == "oxidcreditcard"}]
                    ...
                    <tr onclick="oxid.form.select('paymentid',[{$inptcounter}]);">
                      <td><input style="float:left;padding-right:16px;" id="test_Payment_[{$sPaymentID}]" type="radio" name="paymentid" value="[{$sPaymentID}]" [{if $oView->getCheckedPaymentId() == $paymentmethod->oxpayments__oxid->value}]checked[{/if}]></td>
                      <td id="test_PaymentDesc_[{$smarty.foreach.PaymentSelect.iteration}]" colspan="2"><label><b>[{ $paymentmethod->oxpayments__oxdesc->value}]</b></label></td>
                    </tr>
                    <tr onclick="oxid.form.select('paymentid',[{$inptcounter}]);">
                      <td></td>
                  ...
                  [{else}]
                    <tr onclick="oxid.form.select('paymentid',[{$inptcounter}]);">
                      <td><input id="test_Payment_[{$sPaymentID}]" type="radio" name="paymentid" value="[{$sPaymentID}]" [{if $oView->getCheckedPaymentId() == $paymentmethod->oxpayments__oxid->value}]checked[{/if}]></td>
                      <td id="test_PaymentDesc_[{$smarty.foreach.PaymentSelect.iteration}]" colspan="2"><label><b style="margin-left:8px;">[{ $paymentmethod->oxpayments__oxdesc->value}] [{ if $paymentmethod->dAddPaymentSum }]([{ $paymentmethod->fAddPaymentSum }] [{ $currency->sign}])[{/if}]</b></label></td>
                    </tr>
                ...
                [{else}]
                  <div class="bar prevnext">
                    <div class="right arrowright">
                        <input class="btn" id="test_PaymentNextStepBottom" name="userform" type="submit" value="[{ oxmultilang ident="PAYMENT_NEXTSTEP" }]">
                    </div>
                  </div>
                [{/if}]
